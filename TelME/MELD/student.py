...
def CE_Loss(pred_outs, logit_t, hidden_s, hidden_t, labels):
    ...
    logit_loss = Logit_Loss().to(pred_outs.device)
    logit_loss = logit_loss(pred_outs, logit_t)
    feature_loss = Feature_Loss().to(pred_outs.device)
        ...
        for i_batch, data in enumerate(train_dataloader):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                ...
                if student_type == "audio":
                    batch_input_tokens, batch_attention_masks, batch_audio, batch_labels = batch_input_tokens.to(device), batch_attention_masks.to(device), batch_audio.to(device), batch_labels.to(device)
                    hidden_s, logit_s = model_s(batch_audio)
                    hidden_t, logit_t = model_t(batch_input_tokens, batch_attention_masks)
                else:
                    batch_input_tokens, batch_attention_masks, batch_video, batch_labels = batch_input_tokens.to(device), batch_attention_masks.to(device), batch_video.to(device), batch_labels.to(device)
    ...
    with torch.no_grad():
        for i_batch, data in enumerate(dataloader):            
            ...
            if student_type == "audio":
                ...
                batch_video, batch_labels = batch_video.to(device), batch_labels.to(device)
                hidden_s, logit_s = model_s(batch_video)
...
def main(args):
    ...
    save_audio = os.path.join(project_root, 'models', 'checkpoints', 'student_audio')
    save_video = os.path.join(project_root, 'models', 'checkpoints', 'student_video')
